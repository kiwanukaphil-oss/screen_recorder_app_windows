...
- **M4 (UI, early start)** — WinUI 3 shell builds and runs from the CLI (no Visual Studio needed): source picker (monitors + windows), audio toggles, codec/fps, record + pause buttons with live stats, global stop hotkey, auto-stop surfacing. `dotnet run --project src/Recorder.App` — or use `tools/Recorder.DevCli` for scripting.
